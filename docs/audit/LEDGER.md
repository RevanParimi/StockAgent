...
| AUD-013 | GAP | P2 | `core/delivery` alerts_sent.jsonl | No rotation/truncation — unbounded growth on volume | phc:21 | FIX | FIXED (Wave F 2026-07-17 — emit_alerts rotates the sent-log atomically past 4000 lines, keeping the 2000-line tail the dedupe window already reads) |
| AUD-015 | GAP | P2 | `core/delivery` index/discovery/preopen alerts | Audience hardcoded to default user only | phc:21 | FIX | FIXED (Wave F 2026-07-17 — `emit_alerts_broadcast` fans system alerts to every push-subscribed user + default; 7 sites switched: ops_alerts ×2, index_watch, discovery shelf, preopen reforecast, portfolio quarantine, reconcile drift; per-user emits untouched) |
...
| AUD-022 | GAP | P2 | tests/contract + tests/integration | 19 failures/errors, all tracing to 2 stale-mock roots: AutomobileAgentOrchestrator + SignalAggregator mock targets | apl:18 (T11 gate); phc:19 | FIX (one cleanup ticket) | FIXED (2026-07-29 — test-only re-point after the multi-sector restructure moved the symbols. Roots: (1) API routes resolve the orchestrator via `get_orchestrator(sector)` → analyse patches `backend.sectors.get_orchestrator`, stream patches `services.api.routes.stream.get_orchestrator`; (2) `SignalAggregator`+LLM client live in `base_orchestrator` now, and automobile runs the unified-analyst path so the legacy-worker-pool tests were re-pointed there. test_phase2_api (14), test_phase0 TestOrchestratorUsesOpenAI (2) + progress-callback rewrite (1), test_orchestrator (2). Rider: fixed the sibling event_ingestor stale-date time-bomb. Production code untouched. **Full suite now 2292P/0F/0E — the "known-red baseline (10F/10E)" the whole audit program A/B'd against is RETIRED; future A/B baseline = green.**) |

## API / UI / chat (Phase 8 owners)

| ID | Tag | Sev | Where | Defect | Evidence | Action | Status |
|----|-----|-----|-------|--------|----------|--------|--------|
| AUD-008 | PERF | P3 | `services/api` /performance | Legacy live-mark cost (M4); stale-history quirk right after delete (T10) | apl:17,21 | FIX | FIXED (Wave F 2026-07-17 — response gains `as_of` (the date the market_value figure refers to) so stale-history reads are identifiable; bounded live-mark branch kept, it fires only pre-first-value-point) |
...
| AUD-018 | GAP | P2 | chat system prompt | get_portfolio_brief tool absent from system prompt — chat can't discover the portfolio tool | phc:21 | FIX | FIXED (verified Ph8 2026-07-16 — registered chat tool at ui_data.py `_CHAT_TOOLS`; row updated Wave F) |
...
| ID | Tag | Sev | Where | Defect | Evidence | Action | Status |
|----|-----|-----|-------|--------|----------|--------|--------|
| AUD-026 | OVERENG | P2 | `core/sectors/` | 23 sector skeleton dirs vs 5 live sector impls (`src/backend/sectors/`); memory: toggled off, 8 LLM calls each if enabled | ph0 (ls verified); mem | DELETE (proposal Ph6) | DELETED (Wave E 2026-07-17: core/sectors tree + tier=core dispatch removed) |
| AUD-027 | DEAD | P3 | `src/frontend/web/` | Stub React app; not in Dockerfile COPY set; prototypes are the real UI | ph0; Dockerfile | DELETE | DELETED (Wave E 2026-07-17) |
...
| AUD-029 | DEAD | P3 | `generate_sector_skeletons.py` (repo root) | One-shot codegen script (1,013-line single commit) living at root | ph0; churn scan | DELETE or move to tools/ | DELETED (Wave E 2026-07-17) |
...
| ID | Tag | Sev | Where | Defect | Evidence | Action | Status |
|----|-----|-----|-------|--------|----------|--------|--------|
| AUD-031 | DEAD | P3 | `services/api/user_profile.py` | 58 LOC, zero importers | ph1 | DELETE | DELETED (Wave E 2026-07-17) |
| AUD-032 | DEAD | P2 | `core/config/prompts/` | 22 legacy per-dimension prompt dirs, 185 files / 8,259 LOC, unreachable (prompts route only imports `backend.*`); superseded by unified analyst | ph1 | DELETE | DELETED (Wave E 2026-07-17: imports re-pointed to backend.sectors.automobile.prompts, shared/ kept, 21 dirs deleted) |
| AUD-033 | DEAD | P2 | `src/backend/sectors/` dead subset | 53 files / ~1,460 LOC: legacy fetchers (npa_metrics, rbi_data, mnre_data, transcript, deal_wins), `automobile/pipeline/graph.py`, `schemas/sub_scores.py`; +5 latent prompt files (338 LOC) editable via prompts route but consumed by nothing | ph1 | DELETE (latent prompts: decide in Ph5) | DELETED (Wave E 2026-07-17: fetchers+latent agent/prompt pairs+auto sub_scores; transcript_nlp + 3 sector sub_scores KEPT — live at HEAD, see Wave E note) |
| AUD-034 | DEAD | P3 | `services/clients/alerting.py` | 196 LOC orphan alerting client | ph1 | DELETE | DELETED (Wave E 2026-07-17: client+shim+TestAlertManager) |
| AUD-035 | DEAD | P3 | `core/intelligence/rag/ingestion/` | ingestion.py + ingest_cli.py (304 LOC) unreachable; retriever/vector_store ARE live | ph1 | DELETE or move to tools | DELETED (Wave E 2026-07-17: ingestion+CLI+chunking tests; retriever/vector_store kept) |
| AUD-036 | GAP | P3 | docs (run instructions) | 7× `python -m scripts.daily_review` + 3× `scripts.generate_forecast` reference files that no longer exist; 19× refs to manual run_schedule runner need a currency check | ph1 (grep) | FIX docs | FIXED (Wave F 2026-07-17 — RL_DESIGN.md's 4 stale lines now use `services.scheduler.run_schedule`; repo-wide grep clean outside audit/plan history; run_schedule refs verified current) |
| AUD-037 | OVERENG | P3 | `scripts/api_exploration/` | 7 exploration spikes (1,505 LOC) shipped inside the prod image (`COPY scripts/`) | ph1; Dockerfile | DELETE or move out of image | DELETED (Wave E 2026-07-17) |
...
| AUD-041 | GAP | P2 | news context fetcher (daily review) | "Last 48h context" for TATAMOTORS contained an article dated 2026-05-13 (2 months old) — staleness filter ineffective or label wrong | plog 11:03:11 | VERIFY then FIX | FIXED (Wave G 2026-07-17 — get_news_context now drops dated-but-old AND undated articles outside a `window_days` window (default 3, covers the Monday-reviews-Friday gap); label matches the window; verified prod-confirmed April-ghost class can no longer reach FeedbackAgent context) |
...
| AUD-057 | LOWIMPACT | P3 | `services/data/stores/api_usage.py`, `nse_key_registry.py:117-122`, symbol/company caches | Non-atomic `write_text` + in-process-only threading locks across 2 uvicorn workers + scheduler → lost updates / torn JSON (Serper monthly counter undercounts → false quota headroom; registry/caches degrade gracefully) | ph3 | FIX (shared temp+rename util, one polish commit) | FIXED (Wave F 2026-07-17 — new `core/utils/atomic_io.py` (mkstemp-in-dir + os.replace); 6 writers swapped: api_usage, nse_key_registry, ops_alerts state, symbol_resolver ×3, calendar_updater; torn-JSON class closed, cross-process RMW races remain accepted P3) |
...
| AUD-072 | OVERENG | P3 | `eval/synthetic.py:70-73` + harness ablation | Ablation deltas measure the generator's own hard-coded constants (0.05/0.03/0.02 injected, then "detected"); real-data ablation is a documented no-op — circular instrumentation | ph4 | KEEP generator (schema parity), drop/flag ablation delta reporting | FIXED (Wave F 2026-07-17 — FLAG chosen: every synthetic delta carries a `caveat` field naming it a generator artifact + WARNING at report time; mechanism kept for plumbing/schema parity) |
| AUD-073 | LOWIMPACT | P3 | `algorithms/factor_regime.py` | IIMA factor data frozen at 2023-03 → MOMENTUM/REVERSAL label + agent leniency (0.80/0.85) are constants from 2022-23 presented as live regime awareness; also `requests.get(verify=False)` in prod code (:71-75) | ph4 | FIX (compute WML live from EOD store or DELETE; at minimum verify=True) | FIXED (Wave F 2026-07-17 — KEEP+neutralize: `data_stale` flag (vintage >18mo, unparseable=stale) forces penalty scale 1.0 and labels the prompt context "STALE, background prior only"; verify=True restored; live-WML-from-EOD rejected — ~16-ticker universe would be pseudo-momentum) |
...
| AUD-079 | DEAD | P3 | `core/graphs/` | 4 files / 12 LOC of migration shims re-exporting `backend.shared.pipeline.graphs` (which is LIVE); zero importers of the shim side | ph6 census | DELETE | DELETED (Wave E 2026-07-17) |
| AUD-080 | DEAD | P3 | namespace/shim residue across `src/backend/` | `src/backend/api/` is now just 2 empty `__init__`s (AUD-028's tree already shrunk — resolves that row); `src/backend/scheduler/` 2 empty; `src/backend/intelligence/` 23 empty `__init__`s (restructure skeleton, zero code); `src/backend/shared/` dead subset 10 files/309 LOC: 2-LOC re-export shims with no importers (clients/alerting, clients/tavily_fetcher, data/cache/macro_cache, data/fetchers/{fundamentals,macro,news}, data/stores/score_store) + `config/rag_config.py` (67) + `prompts/feedback_agent.py` (227, superseded by core/config/prompts/shared/feedback_agent.py) + `core/config/rag_config.py` 3-LOC shim | ph6 census | DELETE (~37 files / ~312 LOC, mostly zero-LOC) | DELETED (Wave E 2026-07-17: 28 empty namespace __init__ + tracked __pycache__) |
| AUD-081 | OVERENG | P2 | `tests/unit/` root vs subdirs | **7 test files exist in TWO collected copies** (pre-restructure root + post-restructure subdir): test_config, test_enhancer, test_prompts, test_regime, test_schemas, test_seasonal, test_signal_aggregator (~1,666 duplicated test lines). Actively double-maintained: both test_config copies edited 2026-07-02; test_enhancer copies have DIVERGED (root 2026-05-31/321L vs subdir 2026-05-12/329L — a fix landed in one copy only). Doubles suite time for these modules and guarantees rot | ph6 (md5 + git dates) | FIX (diff each pair, merge newest into the subdir copy, delete root copy) | FIXED (Wave E 2026-07-17: 7 root copies merged into subdir canonicals — enhancer keeps hermetic variant; tests/test_scheduler.py NOT a dup, kept) |
...
| AUD-095 | DEAD | P3 | `graphs/nodes.py:69,340,376` + 16 `core/sectors/*/graph.py` | Legacy LangGraph DAG duplicates resolver+aggregation with a tier twin-drift: its final verdict aggregation runs on LLM_MODEL→BULK while both live verdict paths (unified_analyst, signal_aggregator fallback) use REASONING; scheduler/services never import the graph path (only the 16 sector `graph.py` shells do — Wave-2 shadow-tree candidates) | grep imports; scheduler grep = 0 hits | CONFIRM dead → fold into Wave-2 deletion docket (AUD-025 family); if any sector graph IS reachable its verdict silently runs on deepseek | FIXED (Wave E 2026-07-17: standalone DAG deleted — 20 sector graph shells, resolver/input-rail/aggregate nodes, input/conflict rails; make_dispatch_fn/make_run_agent_node/output_rail KEPT for the live UNIFIED_ANALYST_FALLBACK_LEGACY pool) |
...
| AUD-098 | COST | P3 | `thesis_reviewer.py:224`, `control_lane.py:47-48` | Down-tier candidates: two tiny bounded-JSON judgments (300-token cap, temp 0.1/0.2, output = bool+multiplier / direction+confidence) run per-ticker daily on REASONING (glm-5.2 $0.686/$2.156) where BULK (deepseek $0.09/$0.18) benches 100% JSON-reliable — ~90% unit-cost cut on the two highest-frequency REASONING jobs; `CONTROL_LANE_MODEL` yaml knob already exists for a zero-code A/B | tier table above; llm-tiers memory bench data | EVALUATE (bench rule applies: live json_object smoke via reasoning_bench.py before any swap; control-lane decision AFTER AUD-060/077 RL-semantics verdict — don't retune a metric under audit) | RESOLVED — DOWN-TIER DECLINED (2026-07-29). **control_lane: KEEP on reasoning** (user decision) — it is the scientific duel baseline; a bulk swap breaks edge-metric continuity AND weakens the control → inflates measured edge, for only ~$0.70/mo. **thesis_reviewer: KEEP on reasoning** — live A/B (`scripts/aud098_thesis_tier_bench.py`, real production prompt, 3 scenarios × 3 trials, `outputs/aud098_thesis_bench_2026-07-29.json`) showed bulk (deepseek-v4-flash) at **89% JSON (1 hard parse-fail in 9) + 3× latency w/ 30-40s outliers** vs reasoning glm-5.2 **100% JSON/100% scenario-correct**; a parse-fail silently no-ops the review (`_parse` safe-default = no haircut, no retry), and the call fires only 1-3×/mo so savings ≈ $0.003/mo. Bench NOT cleared → not worth the RL-signal risk. Both call sites stay on `LLM_MODEL_REASONING`. |
...
## Wave E — Dead-code deletion (2026-07-17, branch audit-wave-e-deletions)

Plan: docs/superpowers/plans/2026-07-17-audit-wave-e-deletions.md. 8 commits.
**Net: 570 files changed, −34,828 / +182 lines** (raw lines incl. package-lock etc.;
docket estimate was 559 files / ~24.7K code-LOC). Suite before/after: fail set
IDENTICAL (10F+10E = AUD-022 stale-mock family + 1 event_ingestor date test);
2235→1982 passed = de-duplicated collections + deleted orphan tests, zero new fails.
compileall clean; main/server/scheduler import smoke clean; all Dockerfile COPY
sources still present.

Shipped: core/sectors tree + codegen + langgraph.json + core/graphs (026/029/079);
tier=core dispatch + CoreSectorAdapter removed from registry (026 activation risk);
21 dead prompt dirs, shared/ kept, 8 live imports re-pointed to
backend.sectors.automobile.prompts (032); src/backend/sectors dead subset (033);
api_exploration (037); frontend/web stub (027); rag/ingestion + syntax-broken CLI
(035); alerting client+shim (034); user_profile (031); namespace residue +
tracked __pycache__ (080); 7 duplicate test copies merged (081); standalone
LangGraph DAG deleted with live fallback pool retained (095).

**Census deviations found at HEAD (Ph6 list corrected during execution):**
1. it_sector transcript_nlp agent+prompt are LIVE (registered in it_sector AGENTS)
   — kept, contra Ph1/Ph6 dead list.
2. banking/it/renewable schemas/sub_scores.py are LIVE — unified_analyst.py
   imports them (lines 141/163/189) for per-dimension sub-score models; only
   automobile's copy (superseded by shared/schemas/pipeline.py) was dead.
3. tests/test_scheduler.py is not an AUD-081 duplicate — it holds two unique
   _active_tickers tests; the real pair count was 7, not 8.
4. base_orchestrator imports graphs.nodes at module level — AUD-095's DAG was
   only half-dead: dispatch/run_agent factories power the live legacy-fallback
   pool and were retained; resolver/input-rail/aggregate died with the shells.

Rider fixes: stale @patch("services.clients.alerting.AlertManager") decorators
stripped from tests/contract/test_scheduler.py (module deleted); dead-agent rows
pruned from 3 sector test_agents.py param lists.

## Wave F — polish SHIPPED (2026-07-17, branch audit-wave-f-polish)

Plan `docs/superpowers/plans/2026-07-17-audit-wave-f-polish.md`; TDD, 11 code commits on an
in-repo branch. Eleven P3-class items closed in one sweep:

- **AUD-057** — new `core/utils/atomic_io.py` (`atomic_write_text`/`atomic_write_json`:
  mkstemp-in-target-dir + `os.replace`); the 6 remaining non-atomic writers swapped
  (api_usage, nse_key_registry, ops_alerts state, symbol_resolver ×3 incl. prune path,
  calendar_updater). Torn-JSON class closed; cross-process read-modify-write races remain
  accepted at this scale.
- **AUD-013** — `emit_alerts` rotates the sent-log atomically past 4,000 lines down to the
  2,000-line tail (the exact window `_seen_keys` already reads — rotation can never change
  dedupe behavior).
- **AUD-015** — `PushStore.user_ids()` + `alert_audience()` + `emit_alerts_broadcast()`;
  the 7 system-level alert sites (ops_alerts _emit + partial-output, index_watch, discovery
  shelf, preopen reforecast, portfolio quarantine, reconcile drift) now fan to every
  push-subscribed user plus the default; per-user emits (advisor escalations, lock-in)
  untouched. Known quirk noted in code: >1 audience user duplicates the single global
  mailbox email.
- **AUD-090d** — `services/data/stores/job_outcomes.py` (file-backed, cross-worker);
  `_daily_review_job` records review_date/produced/expected/stragglers/pipeline_ok(+error);
  GET /scheduler/status returns it as `last_runs`. Completes AUD-090 (a–c were Wave A).
- **AUD-091** — `close_on` retries the whole fetch chain once after 2s on the money path;
  `get_price_history` half found ALREADY fixed by code evolution (3-attempt backoff +
  self-heal) — retry-coverage gap is now closed at both cited sites.
- **AUD-089** — value points carry `change_span_days` (calendar-day gap behind
  `day_change_pct`); out-of-order skip kept by design but logs at INFO. **AUD-008** —
  `/portfolio/performance` returns `as_of` naming the date the market_value figure refers to.
- **AUD-073** — factor_regime KEEP+neutralize: `data_stale` (vintage >18 months, unparseable
  = stale) forces `get_regime_penalty_scale` to 1.0 and stamps the prompt line "STALE,
  background prior only"; `verify=False` removed (TLS verification restored). Computing WML
  live from the ~16-ticker EOD store rejected as pseudo-momentum.
- **AUD-072** — FLAG chosen: synthetic ablation deltas carry a `caveat` field naming them
  generator artifacts + a WARNING at report time; mechanism retained for plumbing parity.
- **AUD-104** — one chat memory model: POST /ui/chat now uses the server-side
  `_SESSION_HISTORY` (12-message window, same as /ui/chat/stream), returns `session_id`,
  and ignores client-supplied `history` (injection surface the live UI never used);
  sphere.jsx fallback carries the session id.
- **AUD-036** — RL_DESIGN.md's 4 dead `scripts.daily_review`/`generate_forecast` lines
  now use `services.scheduler.run_schedule`; repo grep clean. AUD-018 row also closed
  (was resolved at Ph8, row still said OPEN).

New tests: test_atomic_io (5), test_atomic_io_adoption (6), test_alerts_wave_f (7),
test_job_outcomes (5), test_pricing_retry (3), test_equity_curve_labels (3),
test_factor_regime_stale (6), harness caveat (1), test_chat_session_memory (3), plus
as_of assertion in test_autopilot_api; index_watch tests repointed to the broadcast seam.
Full suite: 2020 passed / 12 skipped; fail set IDENTICAL to baseline (10F+10E = AUD-022
stale-mock family + event_ingestor date test). Wave F leaves NO open polish rows; the
remaining open program items are AUD-022 (stale-mock cleanup ticket), AUD-019/098 and the
RL-semantics family 060/066/077 (USER-DECISION), AUD-041 (news date filter, P1-candidate,
rides the RL-semantics wave), and the data-gated telemetry $ pull.

## Wave G — RL semantics SHIPPED (2026-07-17, branch audit-wave-g-rl-semantics)

Plan `docs/superpowers/plans/2026-07-17-audit-wave-g-rl-semantics.md`; TDD, 3 code
commits. Executes the user's same-day decisions (AUD-077 → "shadow lane first",
AUD-060 → "fix forward"):

- **AUD-060 FIXED-FORWARD** — `is_direction_correct` NEUTRAL/other branch returns
  `actual_direction == "FLAT"`; live rule now matches the synthetic generator's.
  Stored history untouched (deliberate semantic break in the accuracy series at
  2026-07-17). Ripple accepted: honest (lower) direction accuracy, no more NEUTRAL
  free-pass WeightAdapter credit, stricter advisor ADD gate, slightly more agent
  re-runs on wrong-NEUTRAL days.
- **AUD-077 SHADOW LANE** — new `backend/shared/pipeline/verdict_shadow.py`:
  `verdict_from_composite` (SCORE_THRESHOLDS bands, lo-inclusive, edge resolves up)
  + never-raises JSONL logger appending to data/rl/verdict_shadow.jsonl from
  `SignalAggregator.run` after parse. Observe-only — FinalReport verdict remains
  the LLM's. **Analysis date ~2026-07-31**: divergence rate + per-lane direction
  accuracy (join shadow rows on feedback logs) → hard-bind vs keep; that verdict
  unblocks AUD-098 (tier retune) and shapes AUD-066 (credit-assignment upgrade).
- **AUD-041 FIXED** — `get_news_context(window_days=3)` drops dated-but-old and
  undated articles; the "(last 48h context)" label that shipped April stories into
  the RL training signal is gone; header names the real window.

New tests: test_direction_semantics (5), test_verdict_shadow (5),
