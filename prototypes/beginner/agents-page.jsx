...
function AgentsPage({ goHome, openChat }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg-base)' }}>
      <TopNav active="agents" goAgents={()=>{}} search={search} setSearch={setSearch}/>
      ...
      <main style={{ maxWidth:1280, margin:'0 auto', padding:'24px 32px 96px' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', gap:24, marginBottom:24 }}>
          ...
          <div style={{ display:'flex', gap:12 }}>
            ...
            <Stat label="Total weight"    value={totalWeight.toFixed(2)} hint="Auto-normalized"/>
        ...
        <div style={{ marginTop:32 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:16 }}>
            {agents.map(a => (
              <AgentCard key={a.key} a={a} onToggle={()=>toggle(a.key)} onOpen={()=>setDrawerKey(a.key)}/>
            ))}
          </div>
        </div>
      </main>
...
function AgentCard({ a, onToggle, onOpen }) {
  return (
    <div className="card" style={{
      ...
      <div style={{ padding:18 }}>
        ...
        <div style={{ fontSize:13, color:'var(--ink-2)', lineHeight:1.5, marginBottom:14, minHeight:38 }}>
          {a.beginner}
        </div>
...
function AgentDrawer({ a, onClose, onToggle, onWeight }) {
  return (
    <>
