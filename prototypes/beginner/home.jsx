...
function Home({ goAgents, openChat }) {
  const [tab, setTab] = useStateHome('today');
  const [search, setSearch] = useStateHome('');

  return (
    <div style={{ minHeight:'100vh', background:'var(--bg-base)' }}>
      <TopNav active="home" goAgents={goAgents} search={search} setSearch={setSearch}/>
...
function TopNav({ active, goAgents, search, setSearch }) {
  return (
    <header style={{
      ...
      <div style={{ maxWidth:1280, margin:'0 auto', padding:'14px 32px',
        ...
        <nav style={{ display:'flex', gap:4, marginLeft:24 }}>
          <NavLink active={active==='home'} icon={<Icon.Home size={15}/>}>Home</NavLink>
          <NavLink onClick={goAgents} active={active==='agents'} icon={<Icon.Cpu size={15}/>}>Agents</NavLink>
          <NavLink icon={<Icon.Layers size={15}/>}>Portfolio</NavLink>
          <NavLink icon={<Icon.Compass size={15}/>}>Learn</NavLink>
        </nav>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <div>
              ...
              <div style={{ display:'flex', alignItems:'baseline', gap:8 }}>
                ...
                <span style={{ color:'var(--buy-strong)', fontWeight:700, fontSize:13 }}>+1.24%</span>
              </div>
            </div>
            <span style={{ fontSize:11, color:'var(--ink-3)' }}>30D</span>
          </div>
          <Sparkline values={window.NIFTY_AUTO_HISTORY} height={70} color="var(--cyan)"/>
        </div>
