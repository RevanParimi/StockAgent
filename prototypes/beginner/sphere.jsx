...
function ChatOverlay({ open, onClose, mode='wireframe' }) {
  ...
  const send = async (text) => {
    ...
    setInput('');
    const reply = await window.__sendChat(text);
    setMsgs(m => {
      const updated = [...m];
      const last = updated[updated.length - 1];
      if (last.loading) updated[updated.length - 1] = {from:'bot', text: reply};
      return updated;
    });
  };
      ...
      <div ref={endRef} style={{ flex:1, padding:16, overflowY:'auto', display:'flex', flexDirection:'column', gap:10 }}>
        {msgs.map((m,i) => (
          <div key={i} style={{
            ...
            {m.loading
              ? <><span style={{width:8,height:8,borderRadius:'50%',background:'var(--cyan)',display:'inline-block',animation:'pulse-soft 1s ease-in-out infinite'}}/> Thinking…</>
              : m.text}
          </div>
        ))}
