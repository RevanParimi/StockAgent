...
class TestAutomobileScheduler:
    @patch("tools.scheduler.AutomobileAgentOrchestrator")
    @patch("tools.scheduler.ScoreStore")
    @patch("tools.scheduler.AlertManager")
    @patch("tools.scheduler.BlockingScheduler", create=True)
    def test_run_now_calls_orchestrator_for_each_ticker(
        ...
        # Orchestrator should have been called once for MARUTI
